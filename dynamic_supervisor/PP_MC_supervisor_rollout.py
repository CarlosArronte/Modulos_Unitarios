...
#TODO: modificar esto para crear la ventana dinamica desde la simulacion
...
def load_mc_dropout_model(model_dir, device):
    ...
    scaler_npz_path = os.path.join(model_dir, "scaler_params.npz")

    if os.path.exists(scaler_npz_path):
        scaler_params = np.load(scaler_npz_path)

        x_scaler = NumpyStandardScaler(
            mean=scaler_params["x_mean"],
            scale=scaler_params["x_scale"],
        )

        y_scaler = NumpyStandardScaler(
            mean=scaler_params["y_mean"],
            scale=scaler_params["y_scale"],
        )
